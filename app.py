...
# تابع هوشمند برای مدیریت Others و حفظ اطلاعات
def smart_select(label, options, key):
    ...
    elif curr_val and curr_val != "None": idx = len(opts) - 1 # یعنی Others بوده
...
# ==========================================
    ...
    st.markdown("<h2 style='color:#fff; font-family:Cinzel;'>PROJECT LIBRARY</h2>", unsafe_allow_html=True)
    ...
    if not my_projs:
        ...
        for p in my_projs:
            with st.expander(f"📌 {p['date']} | Final Prompt"):
                st.code(p['prompt'])
...
# ==========================================
    ...
    st.markdown("<h2 style='color:#fff; font-family:Cinzel;'>SYSTEM SETTINGS</h2>", unsafe_allow_html=True)
    st.markdown('<div class="glass-panel">', unsafe_allow_html=True)
    st.selectbox("Default AI Engine", ["Midjourney V6", "Gemini Pro Vision", "Stable Diffusion"])
