...
# --- ماژول Cinematic ---
    ...
    # موتور دیتابیس (برگشت توضیحات تخصصی)
    def add_n(d): 
    ...
    age_d = add_n({"Elderly / Senior": "collagen loss", "Middle-aged": "initial sagging", "Young Adult": "peak elasticity", "Child": "textureless"})
    nat_d = add_n({"Iranian": "Indo-Aryan features", "Saudi": "Peninsular Arab", "European": "Caucasian", "African": "Sub-Saharan"})
    era_d = add_n({"Ancient": "Classical", "Medieval": "Gritty", "100 Years Ago": "Period growth", "Contemporary": "Modern Day"})
    char_d = add_n({"Heroic Warrior": "Strong jawline", "Sinister Villain": "Harsh shadows", "Scholar": "Refined", "Mercenary": "Rugged"})
    groom_d = add_n({"Clean Shaven": "smooth skin", "Light Stubble": "short", "Heavy Stubble": "rough", "Full Beard": "natural growth", "Goatee": "chin beard"})
    ...
    with c_form:
        ...
        with f3:
            ...
            st.markdown('<p class="label-text">Technical <span class="star">*</span></p>', unsafe_allow_html=True)
    ...
    final_p = f"{p_actor}A professional cinematic{p_size} portrait of a {f('', gen_v, gender_d)} {f('', age_v, age_d)} {f('', nat_v, nat_d)}{f(' from ', era_v, era_d)}. {f('Concept: ', char_v, char_d)}. {f('Grooming: ', groom_v, groom_d)}. {f('Hair Color: ', hc_v if hc_v != 'Others' else hc, h_col_d)}. {f('Skin: ', aging_d[list(aging_d.keys())[1]], aging_d)}.{p_sfx} Material: {f('', mat_v if mat_v != 'Others' else mat)}. Tech: {f('', light_v, light_d)}, {f('', cam_v, cam_d)}, 8k raw photography, subsurface scattering."
...
st.markdown(f"<div class='footer'>© {datetime.now().year} <span class='uona-tag'>UONA GROUP</span>. ALL RIGHTS RESERVED</div>", unsafe_allow_html=True)        border-radius: 12px !important;
        font-family: 'Cinzel', serif !important;
        font-weight: 900 !important;
        text-transform: uppercase;
        letter-spacing: 1px;
