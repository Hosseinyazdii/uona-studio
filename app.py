...
# 2. استایل اختصاصی (اصلاح رنگ فیروزه‌ای متوسط برای خوانایی)
st.markdown("""
    <style>
    ...
    /* رنگ فیروزه‌ای متعادل برای تمام لیبل‌ها و متون کوچک */
    .stSelectbox label, .stTextInput label, .label-text { 
        color: #00bcd4 !important; /* فیروزه‌ای متعادل */
...
# --- پورتال لاگین (UI قبلی که تایید شده بود) ---
        ...
        if mode == "Login":
            ...
            if st.button("CREATE ACCOUNT"):
                if u_name and u_pass:
                    save_user(u_name, u_pass); st.success("Registered! Go to Login.")
    st.stop()
...
# --- ماژول Cinematic (12 سوال کامل با دیتای دقیق) ---
    ...
    with t_builder:
        ...
        # فرمول پرامپت دقیق (بازگشت تمام پارامترها)
        final_p = f"Professional cinematic portrait, {size}, {gen}, {age}, {nat}. Concept: {char}, {groom}. Hair: {h_col}. SFX: {sfx}. Material: {mat}. Tech: {cam}, {light}, 8k raw photo, subsurface scattering."
