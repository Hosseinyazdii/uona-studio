...
def generate_prompt(draft):
    ...
    # 🔴 اینجا ترفند اصلی برای Midjourney و SD اجرا میشه 🔴
    if is_arc_active:
        ...
        prompt += "character at different stages. "
    else:
    ...
    if G9 not in ["Feminine / Female", "Female", "Feminine"] and J14_desc:
        prompt += f"Base Grooming: {J14_desc}. "
        
    if J19_desc: prompt += f"Base Hair Texture: {J19_desc}. "
        
    prompt += "Skin: standard. " 
        
    if not is_arc_active and "Child" not in J7 and "Teen" not in J7 and G17_desc:
    ...
    if is_arc_active:
        prompt += "CRITICAL INSTRUCTION: The underlying facial identity, bone structure, and facial proportions MUST remain identical across all three panels. "
        if age_prog_key != "None":
            arc_desc = AGE_PROG_DESC.get(age_prog_key, age_prog_key)
            prompt += f"Timeline Aging Arc: [{arc_desc}]. Apply progressive aging strictly to skin texture, wrinkles, and hair/beard graying. "
            if "Deep Nasolabial Folds" in age_prog_key:
                prompt += "Panel 1 (Left): age 30. Panel 2 (Middle): age 40. Panel 3 (Right): age 50. Add text label at the bottom margin: 'Age 30', 'Age 40', 'Age 50'. "
            else:
                prompt += "Panel 1 (Left): base age. Panel 2 (Middle): +10 years. Panel 3 (Right): +20 years. Add text label at the bottom margin for each age. "

        elif sfx_prog_key != "None":
            prompt += f"SFX Trauma Healing Arc: Base trauma is {G17_desc}. Only progress the healing stages of the trauma. "
            prompt += "Panel 1 (Left): fresh trauma with bleeding. Panel 2 (Middle): scabbed/bruised healing stage. Panel 3 (Right): fully healed scar. Add text label at the bottom margin for each stage. "
    else:
        prompt += "beautifully framed composition, three-quarter profile angle, subsurface scattering, no-retouch. "
        if J7: prompt += f"Typography overlay: clearly written text '{J7}' at the bottom margin of the image. "

    if is_arc_active:
        prompt += "Ensure cinematic raw photography style for all panels, 8k, focus on prosthetic makeup accuracy."
    else:
...
# ==========================================
    ...
    if st.session_state.step == 1:
        ...
        with c2:
            smart_select("Trauma / SFX", list(SFX_DESC        encoded_string = base64.b64encode(f.read()).decode()
